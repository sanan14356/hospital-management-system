...
$formData = $patient;
$formData['assigned_doctor_id'] = $patient['assigned_doctor_id'] ?? '';
...
if (isPost()) {
    ...
    if (empty($errors)) {
        ...
        $currentDoctorId = $patient['assigned_doctor_id'] ? (int)$patient['assigned_doctor_id'] : null;

        $stmt = mysqli_prepare(
            $conn,
            "UPDATE patients
             SET full_name = ?, age = ?, gender = ?, phone = ?, email = ?, address = ?, disease = ?, blood_group = ?, admission_date = ?, assigned_doctor_id = ?
             WHERE id = ?"
        );
        ...
        if (mysqli_stmt_execute($stmt)) {
            if ($assignedDoctorId !== $currentDoctorId) {
                $resetStmt = mysqli_prepare($conn, "UPDATE assignments SET is_active = 0 WHERE patient_id = ? AND is_active = 1");
                mysqli_stmt_bind_param($resetStmt, "i", $patientId);
            ...
            redirect('patient_view.php?id=' . $patientId);
        }

        $errors[] = 'Unable to update patient. Please try again.';
    }
}
...
<div class="page-header">
    <div>
        <h1>Edit Patient</h1>
        <p>Update patient details and doctor assignment.</p>
    </div>
    <a class="btn btn-outline" href="patient_view.php?id=<?php echo $patientId; ?>">Back to Record</a>
</div>
...
<div class="panel">
    ...
    <form method="POST" class="form" data-validate="true">
        ...
        <div class="form-actions">
            ...
            <a class="btn btn-ghost" href="patient_view.php?id=<?php echo $patientId; ?>">Cancel</a>
        </div>
    </form>
</div>
